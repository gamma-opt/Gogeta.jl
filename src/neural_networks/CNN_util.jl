...
function get_structure(CNN_model, input)
    ...
    return CNNStructure(channels, dims, dense_lengths, conv_inds, maxpool_inds, flatten_ind, dense_inds)
end
...
function image_pass!(jump_model, input, cnnstruct, layer)
    ...
    if layer in union(0, cnnstruct.conv_inds, cnnstruct.maxpool_inds) 
